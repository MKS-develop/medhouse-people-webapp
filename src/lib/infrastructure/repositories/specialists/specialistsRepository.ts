...
import { localitySupabaseToMap, specialistSupabaseToMap, specialtySupabaseToMap } from '@/lib/domain/mappers/specialists/supabase/specialistSupabaseMapper';
...
export class SpecialistsRepository implements ISpecialistsRepository {
  async getSpecialists(): Promise<Array<Specialist> | SpecialistsFailure> {
    try {
      ...
      if(error)throw new SpecialistsFailure(error.details)
      
      let list:Specialist[] = data.map(elem => ({
        ...specialistSupabaseToMap({
          ...elem,
          EspecialidadesDoctores: elem["EspecialidadesDoctores"].map((specialty:any)=>
            specialtySupabaseToMap({
              ...specialty,
              nombre: specialty["Especialidades"]["nombre"]
            })
          ),
          Localidades: elem["LocalidadesDoctores"].map((locality:any)=>localitySupabaseToMap({
  ...
  async getSpecialist(id:number): Promise<Specialist | SpecialistsFailure> {
    try {
      ...
      return specialistSupabaseToMap(response.data) ?? {} as Specialist;
    } catch (error) {
      console.log("Error", error)
      const exception = error as any;
      return new SpecialistsFailure(specialistsFailuresEnum.serverError);
